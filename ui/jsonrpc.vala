...
public class RpcClient:GLib.Object{
	...
	public bool get_stranger_msgs_async( callback_add_row add_row ){
        ...
        try{
            ...
            stdout.printf ("get_stranger_msgs_async Error: %s\n", e.message);
            application1.release ();
            return false;
        }
	}
    ...
    public bool get_friends_async(){
        ...
        try{
            ...
            stdout.printf ("get_friends_async Error: %s\n", e.message);
            application1.release ();
            return false;
        }
    }
	...
	//send text message
	public bool multi_send( int64[] to,string msg ){
		var data = new Variant[to.length];
		for(int i=0;i<to.length;i++){
			data[i] = new Variant.int64(to[i]);
		}
		var ids = new Variant.tuple(data);
		var v = new Variant.string("TEXT"+msg);
		var params = new Variant.parsed( "{'Ids':%v,'Msg':%v}",ids,v );
        //Variant res;
        application1.hold ();
        try{
            c.call_async.begin("PClient.MultiSend",params,null,(s,r)=>{c.call_async.end(r,null);});
            application1.release ();
            return true;
        }catch (Error e) {
            stdout.printf ("tell Error: %s\n", e.message);
	...
	public bool delete_me(string name,string pwd){
		var params = new Variant("(ss)",name,pwd);
		application1.hold ();
        try{
